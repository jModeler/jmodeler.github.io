...
I want to choose $\gamma$ in such a way that $E(\epsilon_z'\epsilon_z) = 0$. Said another way, we want to choose our coefficient $\gamma$ that the expected value of the sum of squared error terms $\epsilon_{z,i}$ is zero. When this is true, I have $E(z|x) = \gamma x = E(\alpha_y v + \epsilon_y|x)$. Since $\epsilon_z = z - \gamma x$, I have 
